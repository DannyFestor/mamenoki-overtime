...
<div x-data="{ expanded: true }" class="flex flex-col w-full border-2 border-sky-900">
    ...
    <div class="w-full flex flex-col" x-show="expanded" x-collapse>
        ...
        <div class="flex flex-col w-full sm:hidden divide-y divide-slate-200">
            @foreach($collection as $item)
                <a href="{{ route('overtime.create', ['date' => $item->date->format('Y-m-d')]) }}" class="flex flex-col">
                    ...
                    @if($item->approver)
                    <div class="p-2">
                        承認済み {{ $item->approver->name }} @ <span class="text-sm">{{ $item->approved_at?->format('Y年m月d日 H:i:s') }}</span>
                    </div>
                    @elseif($item->applicant)
                    <div class="p-2">
                        申請済み {{ $item->applicant->name }} @ <span class="text-sm">{{ $item->applied_at?->format('Y年m月d日 H:i:s') }}</span>
                    </div>
                    @else
